...
def solution(str1, str2):
    ...
    for i in range(0, len(str1)-1):
        if all(chr.isalpha() for chr in str1[i:i+2]):
            list_a.append(str1[i:i+2].lower())        
        
    for j in range(0, len(str2)-1):
        if all(chr.isalpha() for chr in str2[j:j+2]):
            list_b.append(str2[j:j+2].lower())
            
    if len(list_a) > len(list_b):
        inter = [list_a.remove(x) for x in list_b if x in list_a]
    else:
        inter = [list_b.remove(x) for x in list_a if x in list_b]
        
    union = len(list_a + list_b)
    if union == 0:
        return 65536
